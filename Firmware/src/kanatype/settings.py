...
Layout (version-tagged; bump MAGIC when it changes):
  ...
  [9:..] 4 profiles x (8-byte name + 8 x (mod bits, key index))
Future consumers (quick-note wake routing etc.) append bytes and bump MAGIC.

The two regions are written independently: save_practice touches only [0:6]
and save_macros only [0:4] and [6:], so neither clobbers the other.
